...
import { ThumbsUpIcon, ThumbsDownIcon } from '../chat-icons';

export function reactionGlyph(key: string): ReactNode {
  if (key === THUMBS_UP_KEY) return <ThumbsUpIcon className="size-3.5" aria-hidden="true" />;
  if (key === THUMBS_DOWN_KEY) return <ThumbsDownIcon className="size-3.5" aria-hidden="true" />;
